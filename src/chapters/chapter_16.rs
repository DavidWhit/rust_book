...
pub fn using_msg_passing_to_trns_data_btwn_threads() {
    ...
    */
    ...
    for _ in 0..10 {
        ...
        // we can't do this because we moved the ownership on the previous version of
        // the loop: Recall back to chapter 15 about Rc<T>
        // compiler catches another issue that it can't safely do this
        // and further that Rc<Mutex<i32>> doesn't implement Send trait
        // Send ensures the types we use with threads are meant for use in
        // conncurrent situations in which Rc<T> is not mean to share thus no Send trait

        // so what can we do?
        // Arc<T> is like Rc<T> but it is safe to use in concurrent situations
        // the A stands for Atomic meaning it's an atomically reference counted type
        // std::sync::atomic for more information

        //So why all primitives types aren't atomic and why the std lib types aren't
        // implemented to use Arc<T>. For this reason thread safety comes with a
        // performance penalty that you only want to pay when you really need to.

        // going back to the code after importing Arc
        let handle = std::thread::spawn(move || {
            let mut num = counter.lock().unwrap();

            *num += 1;
        });
        // finishes closure lock is released because of scope
    ...
    // see changes above with Arc

    // *** conclusion
    /*
    Using this strategy, you can divide a calculation into independent parts,
    split those parts across threads, and then use a Mutex<T> to have each
    thread update the final result with its part.


    Notice that counte ris immutable but we got a mutable reference
        let counter = Arc::clone(&counter);

    Meaning Mutex<T> provides interior mutability as the Cell family does
    In the same way RefCell<T> allows us to mutate contents inside an Rc<T>
    We used Mutext<T> to mutate contents inside an Arc<T>


    Also just like Rust can't protect against Rc<T> having ref cycles
    when lead to memory leaks

    Mutext<T> comes with the risk of deadlocking when an operation needs to lock
    two resources and two threads have each acquired one of the locks causing them to
